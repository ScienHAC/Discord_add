...
const commands = [
  ...
  {
    name: 'remove-user',
    description: 'Remove a user from all channels',
    options: [
      {
        type: 6, // USER type (user ID)
        name: 'usr',
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (commandName === 'add-user') {
    ...
    try {
      ...
      channels.forEach(async (channel) => {
        try {
          ...
          if (!channel || !channel.permissionsFor) {
            failureCount++;
            failureReasons.push(`Invalid channel: ${channel.id}`);
          ...
          if (!botPermissions.has(PermissionsBitField.Flags.ManageRoles)) {
            failureCount++;
            failureReasons.push(`Bot lacks permission to manage roles in channel: ${channel.name}`);
            return; // Skip if the bot doesn't have permissions
          }
      ...
      // Ensure we wait for all edits to complete before replying
    ...
    try {
      ...
      await interaction.followUp(`User ${user.tag} (ID: ${user.id}) has been removed from ${successCount} channels.\n${failureCount > 0 ? `${failureCount} failures (showing first 10):\n${failureMessage}${remainingFailures}` : ''}`);
    } catch (error) {
      console.error('Error removing user from all channels:', error);
      await interaction.followUp('There was an error while trying to remove the user from all channels.');
    }
  }
});
